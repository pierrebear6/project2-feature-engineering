...
def find_stat_series(df: pd.DataFrame,
                     ...
                     diffs: np.linspace = np.linspace(0.05, 0.95, 20),
...
def get_stationary_features(df):
    df = df.copy()
    not_stationary_columns = []
    for column in df.columns:
        result = adfuller(df[column])
        p_value = result[1]
        if p_value > 0.05:
            not_stationary_columns.append(column)

    return not_stationary_columns


def decompose_time_series(df, column_name):
    df = df.copy()
    dum_df = df.copy()
    result = seasonal_decompose(dum_df[column_name], model='multiplicative', extrapolate_trend='freq', period=5)
